		...
		switch (modo)
    {
      case ModoIdle:
       ...
       case ModoTerminal:
					// desliga o timer 0A
          TIMER0_CTL_R = 0x00;
					//Le o angulo digitado pelo usuário
					lerAngulo();
...
void lerAngulo()
	{
		...
		while(!leituraCorreta)//espera o usuário digitar um angulo corretamente 
			{
				//Mostra o angulo digitado pelo usuário
				for(int i = 0; i < 3; i++)
				{                
				...
				{
					empacota_string("+ ou - não encontrado\r\n");
...
//Fiz essa função porque não sabia de que modo eu atualizava o terminal.
...
//	}
	...
	//método para girar o motor em modo terminal
	void girarMotorTerminal()
	{
					if(anguloEscrito[0] == '+')
					{
